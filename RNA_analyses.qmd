...
```{r}
#Figure 1F) Transcriptomic differences between HPV-positive and negative
...
```{r}
#Figure 1G) CDKN2A (p16) expression vs leiden_27 to accurately classify HPV-positive tumors
...
#Figure 1H) DNA repair module (HPV-neg vs pos and GO enriched terms)
...
#Figure 1I) lncRNAs from unsupervised modules higher in HPV-negative samples
...
```{r}
#Figures 1J-K) Differences in immune score between HPV-neg & pos, and taking only M naive patients
full_features %>%
...
```{r}
#Figures S2D) Differences in immune score between HPV-neg and non-smoker non-drinker HPV-pos
full_features[,!(clinical_data_rm$HPV_status_temp == "Positive" & 
                clinical_data_rm$Consumption == "Smoker_and_or_drinker")] %>%
  ggplot(aes(HPV_status, Immune_Score)) + 
  geom_boxplot(linewidth = 0.8) +
  theme_bw(base_size = 30, base_family = "Helvetica") +
  labs(x = "", y = "RNA Immune score") + 
...
library(forcats)
#Figure 2C) Transcriptomic differences between locoregional and distant metastases
changes_univs %>%
...
```{r}
#Figure 2D-E) Correlations around EGFR activty score
...
```{r}
#Figure 2F) Effect of different genes on EGFR pathway activity
...
```{r}
#Figure 2H) Effect of different genes on CXCL12 activity score
...
```{r}
#Figure 3C-D) Representative examples of systemic treatment lines effect on the transcriptome
full_features  %>%
...
```{r}
#Figure S7C) Clustering of transcriptomic features affected by treatment lines
...
```{r}
#Figure S7D) Examples of features increasing with treatment line and high in M naive patients
full_features  %>%
...
#Figure S2C) General heatmap of all features with Keck et al transcriptomic subtypes
...
#Figure S3D) Centroid correlation in our cohort and TCGA
subtype_cors_df %>%
...
```{r}
#Figure S3C) Clinical variables with more effect on the transcriptome
...
```{r}
#Figure S3I) Relation between RNA-IF concordance and immune cell abundance
...
```{r}
#Figures S3J-K) Examples of IF-RNA correlation: T cells and DCs
full_features %>% 
...
```{r}
#Figure S3L) upper half - Relation of RNA Immune score to main genomic parameters
